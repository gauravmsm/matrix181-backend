...
def fetch_full_bhavdata(date):
    """
    sec_bhavdata_full_DDMMYYYY.csv — confirmed working.
    Cols: SYMBOL, SERIES, DATE1, PREV_CLOSE, OPEN_PRICE, HIGH, LOW,
          CLOSE_PRICE, AVG_PRICE, VOLUME, VALUE, NO_OF_TRADES,
          DELIV_QTY, DELIV_PER
    """
...
def fetch_52w_report(date):
    """
    Confirmed URL: https://nsearchives.nseindia.com/content/CM_52_wk_High_low_DDMMYYYY.csv
    Cols: SYMBOL, SERIES, Adjusted 52_Week_High, 52_Week_High_Date,
    ...
    try:
        r = SESSION.get(url, timeout=20)
        print(f"  52W {url[-40:]} → {r.status_code} len={len(r.content)}")
        if r.status_code == 200 and len(r.content) > 10000:
            from io import StringIO
            # File has a disclaimer header — skip first 2 rows
            for enc in ["utf-8", "latin-1"]:
                try:
                    ...
                    for skip in [0, 1, 2, 3]:
                        try:
                            ...
                            # High/Low cols: "ADJUSTED_52_WEEK_HIGH" and "ADJUSTED_52_WEEK_LOW"
                            ...
                            if sym_col and high_col and low_col:
                                print(f"  52W cols (skip={skip}): {list(df.columns)}")
                                df[high_col] = pd.to_numeric(df[high_col].astype(str).str.strip(), errors="coerce")
                                df[low_col]  = pd.to_numeric(df[low_col].astype(str).str.strip(),  errors="coerce")
                                result = {}
                                for _, row in df.iterrows():
                                    sym = str(row[sym_col]).strip().upper().strip('"')
                                ...
                                if len(result) > 100:
                                    print(f"  52W: {len(result)} ✓ sample: {list(result.items())[:3]}")
                                    return result
                        except: continue
                except: continue
    except Exception as e:
...
def check_s1(ltp, mcap, rsi, high_52w, low_52w, avg_d20, avg_d50,
    ...
    # 52W filter: soft — only apply if we have reliable 52W data
...
# ── Fetch full bhavdata for past 5 days (delivery + OHLCV) ────────────────────
print("\nFetching sec_bhavdata_full (delivery + OHLCV)...")
for date in trading_dates:
    ...
    # Latest close from most recent day
    if not latest_close and "CLOSE_PRICE" in eq.columns and "SYMBOL" in eq.columns:
    ...
    # Add to bhavcopy list (normalised)
    df2 = normalise_bhav(df)
    df2["DATE"] = ds
    bhavcopy_list.append(df2)
    print(f"  {ds}: del={len(delivery_map.get(ds,{}))} val={len(value_map.get(ds,{}))} close={len(latest_close) if not latest_close else '(set)'}")

print(f"  Delivery days: {len(delivery_map)} | Closes: {len(latest_close)}")
...
for date in trading_dates:
    ...
    if w52_map: break
print(f"52W: {len(w52_map)} symbols {'✓' if w52_map else '✗ will use extended closes'}")
...
# ── Phase 1: remaining 55 bhavcopy days ───────────────────────────────────────
detail_dates = get_trading_dates(DAYS_DETAIL)
loaded_dates = {b["DATE"].iloc[0] for b in bhavcopy_list if len(b)>0 and "DATE" in b.columns}
cols_printed = False
...
# ── Phase 2: extra close-only days for SMA150/200 ─────────────────────────────
...
for symbol in symbols:
    try:
        ...
        # 52W — dedicated report first, then extended closes fallback
        if w52_avail and sym_u in w52_map:
            high_52w = w52_map[sym_u]["high"]
            low_52w  = w52_map[sym_u]["low"]
        elif len(all_c) >= 100:
            # Use extended closes — reasonable approximation
            year_c   = all_c.tail(252)
            high_52w = round(float(year_c.max()),2)
            low_52w  = round(float(year_c.min()),2)
        else:
            # Not enough data — skip 52W filter by setting values to allow all
            high_52w = None
            low_52w  = None
