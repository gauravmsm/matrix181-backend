...
Data sources:
  1. BhavCopy ZIP × 260 days  → OHLCV, 52W High/Low, Volume, Value
  2. sec_bhavdata_full × 60d  → DELIV_PER (delivery %)
  3. SHARES_MAP (embedded)    → MCap = Close × shares_cr
     Updated from StocksTraded.csv (NSE all-reports page)
"""
import pandas as pd
import numpy as np
...
S1_VOL_RATIO = 1.10    # 20D/50D (soft) — relaxed from 1.20
S1_DEL_MIN   = 40.0    # 20D avg delivery — relaxed from 45%
S1_DEL_GAP   = 2.0     # 20D > 50D + 2% — relaxed from 3%
S1_TV_MIN    = 5.0     # Cr/day — relaxed from 7.5
S1_LOW_MIN   = 1.05;   S1_LOW_MAX  = 1.50   # 52W low — widened to 5-50%
S1_H60       = 0.85    # 60D high — relaxed from 0.90

S2_MCAP_MIN  = 30000
S2_RSI_MIN   = 40.0;   S2_RSI_MAX   = 70.0
S2_VOL_RATIO = 1.10    # soft — relaxed from 1.20
S2_DEL_MIN   = 40.0    # active 10+ days
S2_TV_MIN    = 25.0    # soft
S2_HIGH_MIN  = 0.65;   S2_HIGH_MAX  = 0.95  # 52W high range

DAYS_BHAV = 260
DAYS_FULL =  60

# ── Shares outstanding (Cr) — from StocksTraded.csv dated 04-Jun-2026 ────────
# MCap (Cr) = Close × SHARES_MAP[symbol]
# Update when StocksTraded.csv is re-downloaded from NSE all-reports
SHARES_MAP = {
    "20MICRONS": 3.5286,
    "21STCENMGM": 1.0501,
    "360ONE": 40.6487,
...
def fetch_bhav(date):
    """BhavCopy ZIP → SYMBOL,OPEN,HIGH,LOW,CLOSE,VOLUME,VALUE_RS"""
...
def fetch_full(date):
    """sec_bhavdata_full → SYMBOL,CLOSE,VOLUME,DELIV_PER"""
    ...
    try:
        ...
        rn={}
        for new,cands in [("CLOSE",["CLOSE_PRICE"]),("VOLUME",["TTL_TRD_QNTY"])]:
            if new not in df.columns:
                for c in cands:
                    if c in df.columns: rn[c]=new; break
        df.rename(columns=rn,inplace=True)
...
def sma(s,n):
    c=s.dropna(); return round(float(c.tail(n).mean()),2) if len(c)>=n else None
...
print(f"MCap    : SHARES_MAP ({len(SHARES_MAP)} stocks embedded)")
print("="*60)
print("S1: MCap 1500-30K | RSI 40-70 | Close>30DMA")
print("    Vol≥1.20x | Del≥45%(10+d) | TV≥7.5Cr | 52W 10-40% | 60DH×0.90")
print("S2: MCap>30K | RSI 40-70 | Close>20/50DMA | 20DMA>50DMA")
print("    200DMA(soft) | Vol≥1.20x | Del≥40%(10+d) | TV≥25Cr | 52W 5-35%↓high")

# ── Phase 1: BhavCopy ZIP × 260 days ─────────────────────────────────────────
all_dates=trdates(DAYS_BHAV)
print(f"\nPhase 1: {len(all_dates)} days BhavCopy ZIP...")
bhav_frames={}
...
# ── Phase 2: sec_bhavdata_full × 60 days (delivery) ──────────────────────────
...
if full_frames:
    ...
    if "DELIV_PER" in df_s.columns:
        ...
        samp=df_s[df_s["DELIV_PER"]>0][["SYMBOL","CLOSE","DELIV_PER"]].head(3)
        print(f"  Latest ({s}): {ok} stocks with delivery")
        print(f"{samp.to_string(index=False)}")
...
# Latest close for MCap (use most recent day in BhavCopy)
lat_ds=sorted(bhav_frames.keys())[-1]
lat_df=bhav_frames[lat_ds]
lat_cl={}
...
# All symbols in BhavCopy that also have shares data
symbols_bhav   = set(all_df["SYMBOL"].unique())
symbols_shares = set(SHARES_MAP.keys())
symbols        = list(symbols_bhav & symbols_shares)

print(f"\n{'='*50}")
print(f"BhavCopy days : {len(bhav_frames)}")
print(f"Delivery days : {del_days}")
print(f"Shares map    : {len(SHARES_MAP)} stocks")
print(f"In BhavCopy   : {len(symbols_bhav)}")
print(f"Screening     : {len(symbols)} stocks")
s1_elig=sum(1 for sym in symbols if sym in lat_cl and
            S1_MCAP_MIN<=lat_cl[sym]*SHARES_MAP.get(sym,0)<=S1_MCAP_MAX)
s2_elig=sum(1 for sym in symbols if sym in lat_cl and
            lat_cl[sym]*SHARES_MAP.get(sym,0)>S2_MCAP_MIN)
print(f"  S1 eligible (1500-30K Cr MCap): {s1_elig}")
print(f"  S2 eligible (>30K Cr MCap)    : {s2_elig}")
...
for sym in symbols:
    try:
        ...
        # MCap from SHARES_MAP (shares_cr × latest close)
        ...
        # Traded value (VALUE_RS in rupees → /1e7 = Cr)
        tv=None
        ...
        # Strategy
        st=None
        if chk_s1(ltp,mcap,rv,l52,h52,h60,d20,d50,nd,vr,tv,s30): st="s1"
        elif chk_s2(ltp,mcap,rv,l52,h52,d20,d50,nd,vr,tv,s20,s50,s200): st="s2"
