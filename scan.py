...
def decode_response(r):
    """Decode response handling gzip/deflate/br compression."""
    content_encoding = r.headers.get("Content-Encoding", "").lower()
    raw = r.content

    # Try gzip manually if response is garbled
    if raw and raw[:2] == b'\x1f\x8b':
        try:
            return gzip.decompress(raw).decode("utf-8")
        except: pass

    # requests usually handles this automatically via .text
    # but try explicit decoding
...
def fetch_delivery_nse_api(date):
    """Fetch delivery from NSE Security Archives API with proper decompression."""
    ...
    for attempt in range(2):
        try:
            ...
            print(f"  Archive {date_str} → {r.status_code} len={len(r.content)} enc={r.headers.get('Content-Encoding','none')}")
            ...
            if text.startswith("<") or "<!DOCTYPE" in text[:100]:
                print(f"  HTML response (attempt {attempt+1}) — refreshing session")
                if attempt == 0:
                    get_nse_session()
                    time.sleep(2)
                    continue
            ...
            if not rows:
                print(f"  No data rows. Keys: {list(data.keys()) if isinstance(data,dict) else 'list'}")
                return {}

            print(f"  {len(rows)} rows | keys: {list(rows[0].keys())[:8]}")
            if rows:
                print(f"  Sample row: { {k:v for k,v in list(rows[0].items())[:6]} }")

            result = {}
            for row in rows:
                sym = str(
                    row.get("CH_SYMBOL") or row.get("symbol") or
                    row.get("SYMBOL") or ""
                ).strip().upper()
                if not sym: continue
                ...
                for k in ["CH_DELIVERY_TO_TRADED_QTY","deliveryToTradedQuantity",
                          "DELIVERY_TO_TRADED_QTY","delivery_to_traded_qty","deliveryPct"]:
                ...
                if dpct is None:
                    trd = None; dlv = None
                    for k in ["CH_TOT_TRADED_QTY","totalTradedQuantity","tradedQuantity"]:
                        if k in row and row[k] is not None:
                            try: trd = float(row[k]); break
                            except: pass
                    for k in ["CH_DELIVERABLE_QTY","deliverableQuantity","deliverableQty"]:
                        if k in row and row[k] is not None:
            ...
            if result:
                ...
                print(f"  No delivery pct found in rows")
            return result

        except json.JSONDecodeError as e:
            print(f"  JSON error (attempt {attempt+1}): {e}")
            sample = text[:300] if 'text' in dir() else "no text"
            print(f"  Text sample: {repr(sample[:200])}")
            if attempt == 0:
                get_nse_session()
                time.sleep(2)
        except Exception as e:
...
for i, date in enumerate(detail_dates):
    try:
        ...
        # Delivery — probe first 3, then continue only if working
        if not del_probe_done or len(delivery_map) > 0:
            ...
            if i >= 2:
                del_probe_done = True
                if not delivery_map:
                    print("  NSE delivery not accessible — skipping remaining")
            time.sleep(0.5)
