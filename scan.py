...
def fetch_bhav(date):
    ...
    for url in [
        ...
        try:
            ...
            # Normalise ISIN column name (needed for BSE delivery ISIN bridge)
...
def fetch_full(date):
    """
    Fetch delivery % from BSE (GitHub Actions IPs not blocked by BSE).

    Join chain using ISIN as bridge key (100% accurate, no name guessing):
      NSE BhavCopy → ISIN → NSE SYMBOL         (built in _NSE_ISIN_MAP after Phase 1)
      BSE UDiFF CSV → ISIN → BSE SCRIP_CD
      BSE Gross Del → SCRIP_CD → DELIV_PER

    BSE URL formats (both tried):
      Bhav new:  /download/BhavCopy/Equity/BhavCopy_BSE_CM_0_0_0_{YYYYMMDD}_F_0000.CSV
      Bhav old:  /download/BhavCopy/Equity/EQ{DDMMYY}_CSV.ZIP
      Gross del: /BSEDATA/gross/{YYYY}/SCBSEALL{DDMM}.zip  (confirmed working 2009–present)
    """
    from io import StringIO
    dt_str   = date.strftime("%Y-%m-%d")
    yyyymmdd = date.strftime("%Y%m%d")    # 20260604
    ddmmyy   = date.strftime("%d%m%y")    # 040626
    ddmm     = date.strftime("%d%m")      # 0406
    yr       = date.strftime("%Y")        # 2026
    ...
    def try_zip(content):
        """Return ZipFile or None — catches 'not a zip file' error cleanly."""
        try:
    ...
    # Try new UDiFF plain-CSV format first (BSE switched ~Jul 2024, mirrors NSE)
    # Cols: ISIN_Code, SC_Code, SC_Name, SC_Group, SC_Type, Open, High, Low,
    #       Close, Last, Prev_Close, No._Trades, No._of_Shrs, Net_Turnov
    for bhav_url in [
        ...
        if not r: continue
        try:
            if bhav_url.endswith(".CSV"):
                raw = pd.read_csv(StringIO(r.content.decode("utf-8", errors="replace")))
            else:
                z = try_zip(r.content)
            ...
            # Keep equity only
            for sc_col in ["SC_TYPE","SC_GRP","SC_GROUP"]:
                if sc_col in raw.columns:
                    # SC_TYPE == 'Q' for equity in old format; keep all in new UDiFF
                    break
            # Normalise column names
            rn = {}
            for new, cands in [
                ("SCRIP_CD",  ["SC_CODE","CODE","SCRIP_CD"]),
                ("ISIN",      ["ISIN_CODE","ISIN_NO","ISIN"]),
                ("CLOSE",     ["CLOSE","CLOSE_PRICE"]),
                ("VOLUME",    ["NO_OF_SHRS","NO._OF_SHRS","NO_SHARES","VOLUME"]),
            ...
            if "ISIN" in raw.columns and "SCRIP_CD" in raw.columns:
                valid = raw[raw["ISIN"].astype(str).str.startswith("IN") &
                            raw["SCRIP_CD"].notna()]
    ...
    # Confirmed URL pattern (working since 2009):
    #   /BSEDATA/gross/{YYYY}/SCBSEALL{DDMM}.zip
    # File inside zip: SCBSEALL{DDMM}.DAT  (pipe or comma delimited)
    # Cols: SCRIP_CD, SCRIP_NAME, TRADED_QTY, DELIV_QTY, DELIV_PER
    scrip_to_deliv = {}
    ...
    for del_url in [
        ...
        try:
            ...
            # Try CSV first, then pipe-delimited (DAT files)
            for sep in [",", "|", "\t"]:
                try:
                    raw = pd.read_csv(StringIO(raw_bytes.decode("latin-1", errors="replace")),
                                      sep=sep)
            ...
            rn = {}
            for new, cands in [
                ("SCRIP_CD",  ["SCRIP_CD","SCRIPCD","CODE","SECURITY_CODE","SC_CODE"]),
                ("DELIV_QTY", ["DELIV_QTY","DELIVERABLE_QTY","NET_DQ_VAL"]),
                ("DELIV_PER", ["DELIV_PER","DEL_PER","PERCENT_DELI","% DELI"]),
                ("VOLUME",    ["TRADED_QTY","TRADE_QTY","NO_OF_SHRS","TRDQTY"]),
            ]:
                if new not in raw.columns:
                    for c in cands:
                        if c in raw.columns: rn[c] = new; break
    ...
    # Path A: ISIN → SCRIP_CD → DELIV_PER  (best quality)
    if nse_isin_map and isin_to_scrip and scrip_to_deliv:
        ...
        for isin, nse_sym in nse_isin_map.items():
            ...
            rows.append({"SYMBOL": nse_sym, "CLOSE": np.nan, "DELIV_PER": round(dp, 2), "DATE": dt_str})
        if len(rows) > 100:
            return pd.DataFrame(rows)

    # Path B: Compute from BSE bhav if DELIV_QTY column present
            ...
            if "DELIV_QTY" in bse_bhav_df.columns and "VOLUME" in bse_bhav_df.columns:
                ...
                for _, row in bse_bhav_df.iterrows():
                    ...
                    rows.append({"SYMBOL": nse_sym, "CLOSE": row.get("CLOSE", np.nan),
                                 "DELIV_PER": round(dp, 2), "DATE": dt_str})
                if len(rows) > 100:
                    return pd.DataFrame(rows)
        except Exception as e:
            print(f"    BSE fallback err {dt_str}: {e}")
...
# ── Build ISIN → NSE Symbol map from Phase 1 (used in fetch_full ISIN bridge) ─
_NSE_ISIN_MAP = {}
for _ds in sorted(bhav_frames.keys(), reverse=True):
    ...
    if "ISIN" in _bf.columns and "SYMBOL" in _bf.columns:
        ...
        _tmp = _tmp[_tmp["ISIN"].astype(str).str.startswith("IN")]
