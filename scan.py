...
def fetch_full(date):
    ...
    Primary: PR{DDMMYYYY}.zip — NSE delivery position report
      Cols: Rec Type, Sr No, Symbol, Series, Traded Qty, Deliverable Qty, % Dly Qt To Traded Qty
      Available: ~60-90 days back
    Fallback: sec_bhavdata_full_{DDMMYYYY}.csv
      Cols: SYMBOL, CLOSE_PRICE, TTL_TRD_QNTY, TURNOVER_LACS, DELIV_QTY, DELIV_PER
      Available: ~25 days back
    """
    ds = date.strftime("%d%m%Y")

    # ── Primary: PR delivery ZIP ──────────────────────────────────────────────
    pr_url = f"https://nsearchives.nseindia.com/archives/equities/deliveries/PR{ds}.zip"
    try:
        r = S.get(pr_url, timeout=20)
        if r.status_code == 200 and len(r.content) > 5000:
            z  = zipfile.ZipFile(io.BytesIO(r.content))
            df = pd.read_csv(z.open(z.namelist()[0]), header=None)
            # PR file format: RecType, SrNo, Symbol, Series, TradedQty, DeliverableQty, PctDelivery
            # Filter rows where col[0]=='DR' (delivery record) and col[3]=='EQ'
            ...
            df["CLOSE"]     = np.nan  # PR file has no close price
            df["DATE"]      = date.strftime("%Y-%m-%d")
...
print(f"\nPhase 2: {len(full_dates)} days sec_bhavdata_full...")
full_frames={}
