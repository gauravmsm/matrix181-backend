...
# Install brotli for NSE API decompression
os.system("pip install brotli --break-system-packages -q")
try:
    import brotli
    BROTLI_OK = True
except ImportError:
...
DAYS_DETAIL = 60
DAYS_SMA    = 250   # increased to ensure enough closes for SMA150/200
...
def get_nse_session():
    try:
        ...
        time.sleep(2)
        SESSION.get("https://www.nseindia.com/report-detail/eq_security", timeout=15)
        time.sleep(1)
        r = SESSION.get("https://www.nseindia.com/api/allIndices", timeout=10)
        print(f"NSE session ready (cookies={len(SESSION.cookies)} API={r.status_code})")
...
def decompress_response(r):
    """Decompress brotli/gzip/deflate response content."""
    enc = r.headers.get("Content-Encoding", "").lower()
    raw = r.content

    # Brotli
    if "br" in enc or (raw and raw[:1] == b'\xce'):
        if BROTLI_OK:
            try:
    ...
    # Gzip
    if "gzip" in enc or (raw and raw[:2] == b'\x1f\x8b'):
        try:
            return gzip.decompress(raw).decode("utf-8")
        except: pass

    # requests auto-decompression fallback
    return r.text

def fetch_delivery_nse_api(date):
    """Fetch delivery from NSE Security Archives API."""
    date_str = date.strftime("%d-%m-%Y")
    url = "https://www.nseindia.com/api/historical/securityArchives"
    params = {"from": date_str, "to": date_str, "dataType": "alldata", "series": "EQ"}

    for attempt in range(2):
        try:
            r = SESSION.get(url, params=params, timeout=20)
            enc = r.headers.get("Content-Encoding", "none")
            print(f"  Archive {date_str} → {r.status_code} len={len(r.content)} enc={enc}")

            if r.status_code != 200: continue

            text = decompress_response(r).strip()

            if not text:
                print("  Empty after decompress")
                continue

            # HTML check
            if text.startswith("<") or "<!DOCTYPE" in text[:100]:
                print(f"  HTML response — refreshing session")
                if attempt == 0:
                    get_nse_session(); time.sleep(2); continue
                return {}
            ...
            if not rows:
                print(f"  No rows. Keys: {list(data.keys()) if isinstance(data,dict) else 'list'}")
                return {}
            ...
            result = {}
            for row in rows:
                sym = str(row.get("CH_SYMBOL") or row.get("symbol") or
                          row.get("SYMBOL") or "").strip().upper()
                if not sym: continue

                dpct = None
                for k in ["CH_DELIVERY_TO_TRADED_QTY","deliveryToTradedQuantity",
                          "DELIVERY_TO_TRADED_QTY","deliveryPct"]:
                    if k in row and row[k] is not None:
                        try: dpct = float(row[k]); break
                        except: pass

                if dpct is None:
                    trd = dlv = None
                    for k in ["CH_TOT_TRADED_QTY","totalTradedQuantity"]:
                        if k in row and row[k] is not None:
                            try: trd = float(row[k]); break
                            except: pass
                    for k in ["CH_DELIVERABLE_QTY","deliverableQuantity"]:
                        if k in row and row[k] is not None:
                            try: dlv = float(row[k]); break
                            except: pass
                    if trd and trd > 0 and dlv is not None and dlv >= 0:
                        dpct = round(dlv/trd*100, 2)

                if dpct is not None and 0 < dpct <= 100:
                    result[sym] = round(dpct, 2)

            if result:
                print(f"  Delivery {date_str}: {len(result)} ✓ sample={list(result.items())[:3]}")
            else:
                # Print one sample row to debug field names
                print(f"  No delivery found. Sample row: {dict(list(rows[0].items())[:8])}")
            return result
...
print(f"brotli   : {'OK' if BROTLI_OK else 'MISSING'}")
print("="*60)
...
for i, date in enumerate(detail_dates):
    try:
        ...
        # Delivery API — probe first 3, keep going if working
        if not del_probe_done or len(delivery_map) > 0:
            dd = fetch_delivery_nse_api(date)
            if dd: delivery_map[ds] = dd
            if i >= 2:
                del_probe_done = True
                if not delivery_map:
                    print("  NSE delivery unavailable — skipping remaining")
            time.sleep(0.5)
        ...
        if "VALUE" in bhav_eq.columns and "SYMBOL" in bhav_eq.columns:
            ...
            for idx in bhav_eq.index:
                ...
                if pd.notna(v) and v > 0: vd[syms[idx]] = float(v)/100
...
# Phase 2 — extra days for SMA150/200
# Need 250 total: 60 detail + 190 extra
extra_dates = get_trading_dates(DAYS_SMA)[DAYS_DETAIL:]
print(f"\nPhase 2: {len(extra_dates)} close-only days (targeting 250 total)...")
extra_list = []
...
# Debug: check SMA data availability for first few symbols
sma_debug = {"has_150":0, "has_200":0, "no_150":0, "no_200":0}

results  = []
s1_count = s2_count = processed = 0
sk = {k:0 for k in ["rows","close","mcap","sma","s1","s2","gc"]}
...
for symbol in symbols:
    try:
        ...
        processed += 1
        if processed % 500 == 0:
            print(f"  {processed}/{len(symbols)} q:{len(results)} s1:{s1_count} s2:{s2_count} sma_debug:{sma_debug}")
        ...
        # Track SMA availability
        if sma150 is not None: sma_debug["has_150"]+=1
        else: sma_debug["no_150"]+=1
        if sma200 is not None: sma_debug["has_200"]+=1
        else: sma_debug["no_200"]+=1
        ...
        strategy = None
        if check_s1(ltp,mcap,rsi,from_low_mult,avg_d20,avg_d50,vol_ratio,avg_tv,sma45,sma150,del_avail,val_avail):
            strategy = "s1"
        elif check_s2(ltp,mcap,rsi,high_52w,avg_d20,avg_d50,vol_ratio,avg_tv,sma20,sma50,sma200,del_avail,val_avail):
            strategy = "s2"
