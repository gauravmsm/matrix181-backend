...
def fetch_mcap(close_map):
    """
    Fetch MCap from NSE equity-stockIndices API (Nifty50+Next50+Midcap150+Smallcap250).
    For stocks outside these indices, MCap=None and MCap filter is skipped.
    """
    mcap_out = {}
    ...
    try: nse.get("https://www.nseindia.com", timeout=10)
    except: pass

    for idx in ["NIFTY+50","NIFTY+NEXT+50","NIFTY+MIDCAP+150","NIFTY+SMALLCAP+250","NIFTY+SMALLCAP+50"]:
        url = f"https://www.nseindia.com/api/equity-stockIndices?index={idx}"
        try:
            r = nse.get(url, timeout=15)
            if r.status_code != 200:
                print(f"  {idx}: HTTP {r.status_code}"); continue
            stocks = r.json().get("data", [])
            added = 0
            for s in stocks:
                sym = str(s.get("symbol","")).strip().upper()
                if not sym or sym in mcap_out: continue
                # NSE API fields for market cap (in crores directly in some responses)
                for field in ["totalMarketCap","marketCap","ffmc","pricebandlower"]:
                    val = s.get(field)
                    if val is not None:
                        try:
                            v = float(str(val).replace(",",""))
                            # totalMarketCap is in Rs, convert to Cr
                            if field == "totalMarketCap" and v > 1e9:
                                mcap_out[sym] = round(v/1e7, 2); added+=1; break
                            elif field == "marketCap" and v > 100:
                                mcap_out[sym] = round(v, 2); added+=1; break
                        except: pass
                # If no MCap field found, compute from lastPrice and perShare info
                if sym not in mcap_out:
                    lp = s.get("lastPrice") or s.get("open")
                    pe = s.get("pe") or s.get("eps")  # not useful for MCap
                    # Try stockIndDetails which has 52W data and MCap
                    pass
            print(f"  {idx}: {len(stocks)} stocks, added {added}")
        except Exception as e:
            print(f"  {idx} error: {e}")

    chk = {s: f"{round(mcap_out[s]):,}" for s in ["TCS","MRF","RELIANCE","HDFCBANK","TATAMOTORS"] if s in mcap_out}
    print(f"  MCap via API: {len(mcap_out)} stocks | {chk}")

    # If API gives nothing, fall back to EQUITY_L with raw line parsing
    if len(mcap_out) < 50:
        print("  Trying EQUITY_L raw parse...")
        url2 = "https://nsearchives.nseindia.com/content/equities/EQUITY_L.csv"
        try:
            r2 = S.get(url2, timeout=20)
            if r2.status_code == 200:
                from io import StringIO
                raw = r2.content.decode("latin-1")
                lines2 = raw.splitlines()
                print(f"  EQUITY_L sample: {lines2[1][:150] if len(lines2)>1 else ''}")
                # Read as fixed columns - header has 7 cols
                ...
                if sc and pc and fc:
                    # Clean commas from all string columns
                    df[pc] = pd.to_numeric(df[pc].astype(str).str.replace(",","").str.replace(" ",""), errors="coerce")
                    df[fc] = pd.to_numeric(df[fc].astype(str).str.replace(",","").str.replace(" ",""), errors="coerce")
                    # Print sample
                    for sym_chk in ["TCS","MRF","RELIANCE"]:
                        row = df[df[sc].astype(str).str.strip()==sym_chk]
                        if len(row):
                            print(f"  {sym_chk}: paid={row.iloc[0][pc]} face={row.iloc[0][fc]} close={close_map.get(sym_chk)}")
        except Exception as e:
