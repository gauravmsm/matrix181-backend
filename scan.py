...
def get_nse_session():
    """Establish NSE session with cookies — required for API access."""
    try:
        # Step 1: Get main page cookies
        r1 = SESSION.get("https://www.nseindia.com", timeout=15)
        print(f"  NSE home: {r1.status_code}")
        time.sleep(1)
        # Step 2: Hit the report page to get session cookies
        r2 = SESSION.get("https://www.nseindia.com/report-detail/eq_security", timeout=15)
        print(f"  NSE report: {r2.status_code}")
        time.sleep(1)
        # Step 3: Hit the API once to validate session
        r3 = SESSION.get("https://www.nseindia.com/api/allIndices", timeout=10)
...
def fetch_delivery_from_nse_security_archive(date):
    """
    Fetch delivery data from NSE Security Archives API.
    URL: https://www.nseindia.com/api/historical/securityArchives
    Params: from=DD-MM-YYYY&to=DD-MM-YYYY&symbol=&dataType=alldata&series=EQ
    
    This is the API behind nseindia.com/report-detail/eq_security
    Leaving symbol blank should return all stocks.
    """
    date_str = date.strftime("%d-%m-%Y")

    # Try the bulk endpoint (all symbols)
    urls_params = [
        ("https://www.nseindia.com/api/historical/securityArchives",
         {"from": date_str, "to": date_str, "dataType": "alldata", "series": "EQ"}),
        ("https://www.nseindia.com/api/historical/securityArchives",
         {"from": date_str, "to": date_str, "dataType": "priceVolumeDeliverable", "series": "EQ"}),
        ("https://www.nseindia.com/api/historical/securityArchives",
         {"from": date_str, "to": date_str, "series": "EQ"}),
    ]

    for url, params in urls_params:
        try:
            r = SESSION.get(url, params=params, timeout=20)
            print(f"  Archive {params.get('dataType','default')} → {r.status_code} len={len(r.content)}")

            if r.status_code != 200 or len(r.content) < 50:
                continue

            data = r.json()
            # Response could be list or dict with 'data' key
            rows = data if isinstance(data, list) else data.get("data", [])

            if not rows:
                print(f"  Empty response: {str(data)[:200]}")
                continue

            print(f"  Got {len(rows)} rows, keys: {list(rows[0].keys()) if rows else 'none'}")

            result = {}
            for row in rows:
                sym = str(row.get("symbol","") or row.get("CH_SYMBOL","") or row.get("SYMBOL","")).strip().upper()
                # Try various delivery % column names
                dpct = (row.get("deliveryToTradedQuantity") or
                        row.get("CH_DELIVERY_TO_TRADED_QTY") or
                        row.get("deliveryPct") or
                        row.get("DELIVERY_TO_TRADED_RATIO") or
                        row.get("deliveryPctTrade") or
                        row.get("CH_CHNG_IN_OPN_INTRST"))  # fallback
                trd = (row.get("totalTradedQuantity") or
                       row.get("CH_TOT_TRADED_QTY") or
                       row.get("tradedQuantity"))
                dlv = (row.get("deliverableQuantity") or
                       row.get("CH_DELIVERABLE_QTY") or
                       row.get("deliverableQty"))
                ...
                if sym:
                    if dpct is not None:
                        try:
                            p = float(dpct)
                            if 0 < p <= 100:
                                result[sym] = round(p, 2)
                        except: pass
                    elif trd and dlv:
                        try:
                            t = float(trd); d = float(dlv)
                            if t > 0 and d >= 0:
                                p = round(d/t*100, 2)
                                if 0 < p <= 100:
                                    result[sym] = p
            ...
            if result:
                print(f"  Delivery from API: {len(result)} ✓ sample: {list(result.items())[:3]}")
                return result
...
for i, date in enumerate(detail_dates):
    try:
        ...
        # Delivery from NSE Security Archives API
        # Only try for first 5 days to probe, then use if working
        if i < 5 or len(delivery_map) > 0:
            dd = fetch_delivery_from_nse_security_archive(date)
            if dd:
                delivery_map[ds] = dd
