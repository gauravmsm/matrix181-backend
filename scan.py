...
def compute_atr(sdf, n=20):
    ...
    try:
        ...
        if len(tr_vals) < n:
            return None
        return round(float(sum(tr_vals[-n:]) / n), 2)
    except:
        return None
...
def compute_targets(ltp, sdf, s200, h52, h60, st):
    ...
    if atr is None or atr <= 0:
        atr = round(ltp * 0.02, 2)
        method = "volatility_proxy"
    else:
        method = "atr"

    t1 = round(ltp + 2 * atr, 2)
    t2 = round(ltp + 4 * atr, 2)
    t3 = round(h52, 2) if (h52 and ltp < h52 * 0.97) else round(ltp + 6 * atr, 2)

    stop_px = round(s200 * 0.97, 2) if s200 else round(ltp * 0.93, 2)
    atr_pct = round(atr / ltp * 100, 2) if ltp > 0 else None
    t1_pct  = round((t1 - ltp) / ltp * 100, 1) if ltp > 0 else None
    t2_pct  = round((t2 - ltp) / ltp * 100, 1) if ltp > 0 else None
    t3_pct  = round((t3 - ltp) / ltp * 100, 1) if ltp > 0 else None

    risk   = ltp - stop_px
    reward = t1  - ltp
    rr     = ("1:" + str(round(reward/risk, 1))) if risk > 0 else None
