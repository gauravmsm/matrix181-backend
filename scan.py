...
def get_nse_session():
    """Full NSE session setup with cookie chain."""
    try:
        # Step 1: Main page
        SESSION.get("https://www.nseindia.com", timeout=15)
        time.sleep(2)
        # Step 2: Report page (sets specific cookies)
        SESSION.get("https://www.nseindia.com/report-detail/eq_security", timeout=15)
        time.sleep(1)
        # Step 3: Validate with allIndices
...
def fetch_delivery_nse_api(date):
    ...
    Handles HTML redirect by re-establishing session.
    """
    ...
    for attempt in range(2):
        try:
            ...
            # If HTML returned — session expired, refresh
            if text.startswith("<") or "<!DOCTYPE" in text[:50]:
                if attempt == 0:
                    print(f"  Got HTML response — refreshing session...")
                    get_nse_session()
                    time.sleep(2)
                    continue
                else:
                    print(f"  Still HTML after refresh — NSE blocking API")
            ...
            if not rows:
                print(f"  Empty data. Response: {str(data)[:200]}")
                return {}

            # First time — print sample to understand structure
            if date == sorted([date], key=lambda d: d)[0]:
                print(f"  API cols: {list(rows[0].keys())}")
            ...
            print(f"  Delivery {date_str}: {len(result)} symbols ✓")
            if result:
            ...
            print(f"  Raw preview: {text[:200] if text else 'empty'}")
            if attempt == 0:
                get_nse_session()
                time.sleep(2)
        except Exception as e:
...
for i, date in enumerate(detail_dates):
    try:
        ...
        # Delivery — try API for first 3 days, then skip if not working
        if not del_probe_done or len(delivery_map) > 0:
            ...
            if i >= 2:
                del_probe_done = True
                if not delivery_map:
                    print("  NSE API not returning delivery — skipping for remaining days")
            time.sleep(0.5)
