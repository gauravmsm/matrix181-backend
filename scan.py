...
DAYS_FULL =  30  # sec_bhavdata_full (recent, has direct DELIV_PER)
...
print(f"  Bhavcopy (RSVD1/VOL): {bhav_del_days} days with delivery")
...
for sym in symbols:
    try:
        ...
        # Traded value (VALUE_L in lakhs → /100 = Cr)
        tv = None
        if "VALUE_L" in sdf.columns:
            tvs = pd.to_numeric(sdf["VALUE_L"],errors="coerce").dropna()
            if len(tvs)>=5: tv = round(float(tvs.tail(20).mean())/100,2)
