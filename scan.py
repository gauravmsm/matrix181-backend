    ...
    try:
        ...
        for enc in ["utf-8","latin-1","cp1252"]:
            try:
                ...
                # Debug: print column names and TCS row
                print(f"  EQUITY_L cols: {list(df.columns)}")
                print(f"  Using: sym={sc} paid={pc} face={fc}")
                tcs = df[df[sc].astype(str).str.strip()=="TCS"]
                if len(tcs):
                    print(f"  TCS row: {dict(tcs.iloc[0][[sc,pc,fc]])}")
                mrf = df[df[sc].astype(str).str.strip()=="MRF"]
                if len(mrf):
                    print(f"  MRF row: {dict(mrf.iloc[0][[sc,pc,fc]])}")
                out = {}
