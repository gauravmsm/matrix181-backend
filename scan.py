...
======================================================
Data: BhavCopy ZIP × 260 days (NSE) + BSE Gross Deliverables × 50 days + SHARES_MAP

Delivery join strategy: ISIN bridge (100% accurate)
  NSE BhavCopy  → ISIN → NSE SYMBOL
  BSE BhavCopy  → ISIN → BSE SCRIP_CD
  BSE GrossDeliv→ SCRIP_CD → DELIV_PER
...
Note: NSE archives block GitHub Actions IPs — delivery sourced from BSE.
"""
import pandas as pd
import numpy as np
...
DAYS_BHAV = 260
DAYS_FULL =  50   # BSE gross deliverables available ~50 trading days back
...
def fetch_bhav(date):
    ...
    for url in [
        ...
        try:
            ...
            # Preserve ISIN if present (used for BSE delivery symbol mapping)
            for isin_col in ["ISIN","ISIN_CODE","ISIN_NUMBER"]:
                if isin_col in df.columns and isin_col != "ISIN":
                    df.rename(columns={isin_col: "ISIN"}, inplace=True)
                    break
...
def fetch_full(date):
    """
    Fetch delivery % data from BSE using ISIN as the bridge key.

    Join chain (100% accurate, no name guessing):
      NSE BhavCopy  → ISIN → NSE SYMBOL   (built once in Phase 1)
      BSE BhavCopy  → ISIN → BSE SCRIP_CD
      BSE Gross Del → SCRIP_CD → DELIV_PER

    Falls back to BSE bhav DELIV_QTY/VOLUME computation if gross del ZIP unavailable.
    """
    from io import StringIO
    dt_str   = date.strftime("%Y-%m-%d")
    ddmmyy   = date.strftime("%d%m%y")    # e.g. 040626
    ddmmyyyy = date.strftime("%d%m%Y")    # e.g. 04062026
    yr       = date.strftime("%Y")
    ...
    # ── Step 1: BSE BhavCopy → ISIN + SCRIP_CD mapping ───────────────────────
    # URL: https://www.bseindia.com/download/BhavCopy/Equity/EQ{DDMMYY}_CSV.ZIP
    # Cols: CODE(=scrip_cd), NAME, SC_GROUP, SC_TYPE, OPEN,HIGH,LOW,CLOSE,
    #       LAST, PREVCLOSE, NO_TRADES, NO_OF_SHRS, NET_TURNOV, TDCLOINDI, ISIN
    isin_to_scrip = {}   # ISIN → BSE scrip code
    bse_bhav_df   = None

    r = bse_get(f"https://www.bseindia.com/download/BhavCopy/Equity/EQ{ddmmyy}_CSV.ZIP")
    if r:
        try:
            z   = zipfile.ZipFile(io.BytesIO(r.content))
            raw = pd.read_csv(z.open(z.namelist()[0]))
            raw.columns = raw.columns.str.strip().str.upper()
            # Keep equity scrips only (SC_TYPE == Q means equity)
            if "SC_TYPE" in raw.columns:
                raw = raw[raw["SC_TYPE"].astype(str).str.strip() == "Q"].copy()
            # Normalise columns
            rn = {}
            for new, cands in [
                ("SCRIP_CD", ["CODE","SCRIP_CD"]),
                ("VOLUME",   ["NO_OF_SHRS","NO_SHARES"]),
                ("DELIV_QTY",["DELIV_QTY"]),
            ]:
    ...
    # URL: https://www.bseindia.com/BSEDATA/gross/{YYYY}/GrossDeliverables_{DDMMYYYY}.zip
    # Cols: SCRIP_CD, SCRIP_NAME, TRADED_QTY, DELIV_QTY, DELIV_PER
    scrip_to_deliv = {}   # BSE scrip code → DELIV_PER

    for del_url in [
        f"https://www.bseindia.com/BSEDATA/gross/{yr}/GrossDeliverables_{ddmmyyyy}.zip",
        f"https://www.bseindia.com/download/Bhavcopy/Equity/GrossDeliverables_{ddmmyyyy}.zip",
        f"https://www.bseindia.com/BSEDATA/gross/GrossDeliverables_{ddmmyyyy}.zip",
    ]:
        r = bse_get(del_url)
        if not r: continue
        try:
            ...
            rn = {}
            for new, cands in [
                ("SCRIP_CD",  ["SCRIP_CD","SCRIPCD","CODE","SECURITY_CODE"]),
                ("DELIV_QTY", ["DELIV_QTY","DELIVERABLE_QTY","NET_DQ_VAL_MTM"]),
                ("DELIV_PER", ["DELIV_PER","DEL_PER","PERCENT_DELI"]),
                ("VOLUME",    ["TRADED_QTY","TRADE_QTY","NO_OF_SHRS"]),
            ]:
                if new not in raw.columns:
                    for c in cands:
                        if c in raw.columns: rn[c] = new; break
            ...
            for col in ["SCRIP_CD","DELIV_QTY","DELIV_PER","VOLUME"]:
                if col in raw.columns:
                    raw[col] = pd.to_numeric(raw[col], errors="coerce")
            # Compute DELIV_PER if missing
            if "DELIV_PER" not in raw.columns:
                if "DELIV_QTY" in raw.columns and "VOLUME" in raw.columns:
    ...
    # ── Step 3: Build ISIN → NSE_SYMBOL map from Phase 1 bhav frames ─────────
    # _NSE_ISIN_MAP is built once (below, after Phase 1) and reused for all days
    # Access it as a module-level dict
    nse_isin_map = globals().get("_NSE_ISIN_MAP", {})

    # ── Step 4: Assemble output using ISIN bridge ─────────────────────────────
    # Path A: ISIN → SCRIP_CD → DELIV_PER → NSE SYMBOL (best quality)
    if nse_isin_map and isin_to_scrip and scrip_to_deliv:
        ...
        for isin, nse_sym in nse_isin_map.items():
            ...
            if dp is None or pd.isna(dp) or dp <= 0 or dp > 100: continue
            rows.append({"SYMBOL": nse_sym, "CLOSE": np.nan, "DELIV_PER": dp, "DATE": dt_str})
        if len(rows) > 100:
            return pd.DataFrame(rows)

    # Path B: Fallback — compute from BSE bhav DELIV_QTY/VOLUME if available
    # (some BSE bhav files include DELIV_QTY directly)
        ...
        try:
            if "DELIV_QTY" in bse_bhav_df.columns and "VOLUME" in bse_bhav_df.columns:
                ...
                for _, row in bse_bhav_df.iterrows():
                    ...
                    if pd.isna(dp) or dp <= 0 or dp > 100: continue
                    rows.append({"SYMBOL": nse_sym, "CLOSE": row.get("CLOSE", np.nan),
                                 "DELIV_PER": dp, "DATE": dt_str})
                if len(rows) > 100:
                    return pd.DataFrame(rows)
        except Exception as e:
...
# ── Build ISIN → NSE Symbol map from Phase 1 data (used by fetch_full) ───────
# Use the most recent bhav day that has an ISIN column
_NSE_ISIN_MAP = {}
for _ds in sorted(bhav_frames.keys(), reverse=True):
    ...
    if "ISIN" in _bf.columns and "SYMBOL" in _bf.columns:
        ...
        break
print(f"  ISIN map: {len(_NSE_ISIN_MAP)} NSE symbols mapped (for BSE delivery join)")
