...
def fetch_full_bhavdata(date):
    """
    NSE Full Bhavcopy with Delivery — confirmed working URL.
    Cols: SYMBOL, SERIES, DATE1, PREV_CLOSE, OPEN_PRICE, HIGH_PRICE,
...
def fetch_52w_report(date):
    ...
    Confirmed URL pattern from NSE all-reports page:
    CM_52_wk_High_low_DDMMYYYY.csv
    """
    d = date.strftime("%d%m%Y")
    # Try multiple URL patterns
    urls = [
        f"https://nsearchives.nseindia.com/content/cm/CM_52_wk_High_low_{d}.csv",
        f"https://nsearchives.nseindia.com/archives/equities/52wkhl/CM_52_wk_High_low_{d}.csv",
        f"https://www.nseindia.com/content/cm/CM_52_wk_High_low_{d}.csv",
        f"https://nsearchives.nseindia.com/content/equities/CM_52_wk_High_low_{d}.csv",
    ]
    for url in urls:
        try:
            r = SESSION.get(url, timeout=20)
            print(f"  52W {url[-50:]} → {r.status_code} len={len(r.content)}")
            if r.status_code == 200 and len(r.content) > 10000:
                from io import StringIO
                for enc in ["utf-8", "latin-1"]:
                    try:
                        ...
                        if sym_col and high_col and low_col:
                            ...
                            print(f"  52W: {len(result)} symbols ✓ sample: {list(result.items())[:3]}")
                            return result
                    except: continue
        except: pass
    return {}
...
# ── Step 1: Fetch full bhavdata (delivery) for recent days ────────────────────
trading_dates = get_trading_dates(5)
delivery_map  = {}
value_map     = {}
latest_close  = {}
latest_date   = None

print("\nFetching sec_bhavdata_full for delivery history...")
for date in trading_dates:
    ...
    if latest_date is None:
        latest_date = date
        print(f"  Latest date: {ds} ({len(df)} rows)")
    ...
    if "DELIV_PER" in eq.columns and "SYMBOL" in eq.columns:
        ...
        if mask.sum() > 100:
            dd = {syms[i]: round(float(dpct[i]),2) for i in eq[mask].index}
            delivery_map[ds] = dd
            print(f"  Delivery {ds}: {len(dd)} ✓")

    # Value (TURNOVER_LACS = lakhs, /100 = crores)
    val_col = next((c for c in ["TURNOVER_LACS","TOTTRDVAL","VALUE"] if c in eq.columns), None)
    ...
    # Latest closes
    if ds == (trading_dates[0]).strftime("%Y-%m-%d") and "CLOSE_PRICE" in eq.columns and "SYMBOL" in eq.columns:
        cl   = pd.to_numeric(eq["CLOSE_PRICE"], errors="coerce")
        syms = eq["SYMBOL"].astype(str).str.strip().str.upper()
        for i in eq.index:
            c = cl[i] if i in cl.index else np.nan
            if pd.notna(c) and c > 0: latest_close[syms[i]] = float(c)

print(f"  Delivery days: {len(delivery_map)} | Value days: {len(value_map)} | Closes: {len(latest_close)}")

# ── Step 2: 52W report ────────────────────────────────────────────────────────
print("\nFetching 52W High Low report...")
w52_map  = {}
w52_date = latest_date or trading_dates[0]
w52_map  = fetch_52w_report(w52_date)
if not w52_map:
    # Try yesterday
    for d in trading_dates[1:3]:
        w52_map = fetch_52w_report(d)
        if w52_map: break
print(f"52W: {len(w52_map)} symbols {'✓' if w52_map else '✗'}")

# ── Step 3: 60 days bhavcopy for OHLCV + SMA data ────────────────────────────
detail_dates = get_trading_dates(DAYS_DETAIL)
bhavcopy_list = []
cols_printed  = False

# Add full bhav data for days we already have
for date in trading_dates:
    df = fetch_full_bhavdata(date)
    if df is not None:
        df2 = normalise_bhav(df)
        df2["DATE"] = date.strftime("%Y-%m-%d")
        bhavcopy_list.append(df2)
        if not cols_printed:
            print(f"\nFull bhav cols: {list(df2.columns)}")
            cols_printed = True

print(f"\nPhase 1: {len(detail_dates)} detail days...")
for i, date in enumerate(detail_dates):
    ds = date.strftime("%Y-%m-%d")
    # Skip if already loaded from full bhav
    if any(b["DATE"].iloc[0] == ds for b in bhavcopy_list if len(b)>0 and "DATE" in b.columns):
        continue
    try:
        ...
        # Also extract delivery/value from regular bhavcopy if available
...
# MCap
if not latest_close:
    latest_close = {k: w52_map[k]["high"] for k in w52_map}  # fallback
...
print(f"Delivery : {len(delivery_map)} days ({'✓' if del_avail else '✗'})")
...
all_bhav = all_bhav.drop_duplicates(subset=["SYMBOL","DATE"], keep="last")
...
all_closes = all_closes.drop_duplicates(subset=["SYMBOL","DATE"], keep="last")
...
for symbol in symbols:
    try:
        ...
        # 52W — from dedicated report, then fallback
        if w52_avail and sym_u in w52_map:
            high_52w = w52_map[sym_u]["high"]
            low_52w  = w52_map[sym_u]["low"]
        else:
            # Use max/min of all available close data
            h_all = all_c if len(all_c) > 0 else closes
            high_52w = round(float(h_all.max()),2)
            low_52w  = round(float(h_all.min()),2)
