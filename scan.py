"""
MATRIX 18.2 — NSE Institutional Accumulation Screener
======================================================
Data: BhavCopy ZIP × 260 days + sec_bhavdata_full × 60 days + SHARES_MAP

S1 Hard: MCap 1500-30K | RSI >40 (no ceiling) | Close>30DMA | 30DMA>150DMA | 150DMA Rising
S2 Hard: MCap >30K     | RSI >40 (no ceiling) | Close>20DMA | Close>50DMA

Soft filters, grading, confidence per MATRIX 18.2 spec.
"""
import pandas as pd
import numpy as np
...
S1_RSI_MIN   = 40.0        # Hard floor only — ceiling moved to confidence score
# Soft
S1_VOL_RATIO = 1.10
S1_TV_MIN    = 5.0
...
# S2 — Large Cap
S2_MCAP_MIN  = 30000
S2_RSI_MIN   = 40.0        # Hard floor only — ceiling moved to confidence score
# Soft
S2_VOL_RATIO = 1.10
S2_TV_MIN    = 25.0
S2_HIGH_MIN  = 0.55;   S2_HIGH_MAX  = 0.98
S2_DEL_MIN   = 40.0

DAYS_BHAV = 260
DAYS_FULL =  50   # BSE gross deliverables ~50 trading days back
...
def fetch_bhav(date):
    ...
    for url in [
        ...
        try:
            ...
            # Normalise ISIN column name if present (needed for BSE delivery bridge)
            for isin_col in ["ISIN_CODE","ISIN_NUMBER","ISINCODE"]:
                if isin_col in df.columns:
                    df.rename(columns={isin_col: "ISIN"}, inplace=True); break
            df["DATE"]=date.strftime("%Y-%m-%d")
            return df
        except: pass
    return None

def _read_bse_csv(content_bytes):
    """
    Robustly parse a BSE CSV/DAT file.
    BSE files often have a multi-line text header (e.g. 6 rows of metadata)
    before the actual column-header row. Strategy:
      1. Decode with latin-1 (BSE files are never UTF-8 strict)
      2. Scan lines to find the first line that looks like a header
         (contains a comma or tab AND has alphabetic tokens)
      3. Re-read from that line with sep auto-detection
    """
    from io import StringIO
    text = content_bytes.decode("latin-1", errors="replace")
    lines = text.splitlines()

    # Find the header row: first line with >= 3 comma/tab-separated tokens
    # that are mostly alphabetic (not all digits)
    header_row = 0
    for i, line in enumerate(lines[:20]):   # only scan first 20 lines
        for sep in [",", "\t", "|"]:
            parts = line.split(sep)
            if len(parts) >= 4 and any(p.strip().replace("_","").replace(" ","").isalpha()
                                        for p in parts):
                header_row = i
                detected_sep = sep
                break
    ...
    # Try detected sep, then others
    for sep in [detected_sep, ",", "\t", "|"]:
        try:
...
def fetch_full(date):
    """
    Fetch delivery % from BSE using ISIN as bridge key (100% accurate).

    Join chain:
      _NSE_ISIN_MAP: ISIN → NSE SYMBOL          (built after Phase 1)
      BSE Bhav CSV:  ISIN → SCRIP_CD
      BSE Gross Del: SCRIP_CD → DELIV_PER

    BSE URLs tried:
      Bhav new UDiFF (plain CSV, no zip, ~Jul 2024+):
        /download/BhavCopy/Equity/BhavCopy_BSE_CM_0_0_0_{YYYYMMDD}_F_0000.CSV
      Bhav old ZIP:
        /download/BhavCopy/Equity/EQ{DDMMYY}_CSV.ZIP
      Gross delivery ZIP (confirmed working 2009–present):
        /BSEDATA/gross/{YYYY}/SCBSEALL{DDMM}.zip
    """
    ...
    def bse_get(url, timeout=25):
        try:
            r = requests.get(url, headers=BSE_HDR, timeout=timeout)
            if r.status_code == 200 and len(r.content) > 500:
                return r
        except Exception as e:
            print(f"    BSE fetch err {url[-55:]}: {e}")
        return None

    def safe_zip(content):
        try:
            return zipfile.ZipFile(io.BytesIO(content))
        except Exception:
            return None

    # ── Step 1: BSE BhavCopy → ISIN + SCRIP_CD ───────────────────────────────
    isin_to_scrip = {}
    bse_bhav_df   = None

    for bhav_url, is_zip in [
        (f"https://www.bseindia.com/download/BhavCopy/Equity/BhavCopy_BSE_CM_0_0_0_{yyyymmdd}_F_0000.CSV", False),
        (f"https://www.bseindia.com/download/BhavCopy/Equity/EQ{ddmmyy}_CSV.ZIP", True),
    ]:
        r = bse_get(bhav_url)
        if not r: continue
        try:
            if is_zip:
                z = safe_zip(r.content)
                if z is None: continue
                raw_bytes = z.open(z.namelist()[0]).read()
            else:
                raw_bytes = r.content

            raw = _read_bse_csv(raw_bytes)
            if raw is None: continue
            raw.columns = raw.columns.str.strip().str.upper()

            # Normalise column names across old and new BSE formats
            rn = {}
            for new, cands in [
                ("SCRIP_CD",  ["SC_CODE","CODE","SCRIP_CD","SECURITY_CODE"]),
                ("ISIN",      ["ISIN_CODE","ISIN_NO","ISIN"]),
                ("CLOSE",     ["CLOSE","CLOSE_PRICE"]),
                ("VOLUME",    ["NO_OF_SHRS","NO._OF_SHRS","NO_SHARES","VOLUME","TRDQTY"]),
                ("DELIV_QTY", ["DELIV_QTY","DELIVERABLE_QTY","NET_DELIV_QTY"]),
            ]:
                if new not in raw.columns:
                    for c in cands:
                        if c in raw.columns: rn[c] = new; break
            raw.rename(columns=rn, inplace=True)
            for col in ["SCRIP_CD","CLOSE","VOLUME"]:
                if col in raw.columns:
                    raw[col] = pd.to_numeric(raw[col], errors="coerce")

            if "ISIN" in raw.columns and "SCRIP_CD" in raw.columns:
                valid = raw[raw["ISIN"].astype(str).str.strip().str.match(r"^IN[A-Z0-9]{10}$") &
                            raw["SCRIP_CD"].notna()]
                if len(valid) > 100:
                    isin_to_scrip = dict(zip(
                        valid["ISIN"].astype(str).str.strip(),
                        valid["SCRIP_CD"].astype(int)
                    ))
                    bse_bhav_df = raw.copy()
                    break
        except Exception as e:
            print(f"    BSE bhav parse err {dt_str}: {e}")

    # ── Step 2: BSE Gross Deliverables → SCRIP_CD + DELIV_PER ────────────────
    scrip_to_deliv = {}

    for del_url in [
        f"https://www.bseindia.com/BSEDATA/gross/{yr}/SCBSEALL{ddmm}.zip",
        f"https://www.bseindia.com/BSEDATA/gross/{yr}/GrossDeliverables_{yyyymmdd}.zip",
        f"https://www.bseindia.com/download/Bhavcopy/Equity/GrossDeliverables_{yyyymmdd}.zip",
    ]:
        r = bse_get(del_url)
        if not r: continue
        z = safe_zip(r.content)
        ...
        try:
            ...
            rn = {}
            for new, cands in [
                ("SCRIP_CD",  ["SCRIP_CD","SCRIPCD","CODE","SC_CODE","SECURITY_CODE"]),
                ("DELIV_QTY", ["DELIV_QTY","DELIVERABLE_QTY","NET_DQ_VAL"]),
                ("DELIV_PER", ["DELIV_PER","DEL_PER","PERCENT_DELI","% DELI","DELI_PER"]),
                ("VOLUME",    ["TRADED_QTY","TRADE_QTY","NO_OF_SHRS","TRDQTY","VOLUME"]),
            ]:
                if new not in raw.columns:
                    for c in cands:
                        if c in raw.columns: rn[c] = new; break
            raw.rename(columns=rn, inplace=True)
            for col in ["SCRIP_CD","DELIV_QTY","DELIV_PER","VOLUME"]:
                if col in raw.columns:
                    raw[col] = pd.to_numeric(raw[col], errors="coerce")
            if "DELIV_PER" not in raw.columns and "DELIV_QTY" in raw.columns and "VOLUME" in raw.columns:
                raw["DELIV_PER"] = (raw["DELIV_QTY"] / raw["VOLUME"] * 100).round(2)
            if "DELIV_PER" in raw.columns and "SCRIP_CD" in raw.columns and len(raw) > 100:
                scrip_to_deliv = dict(zip(
                    raw["SCRIP_CD"].dropna().astype(int),
                    raw["DELIV_PER"]
                ))
                break
        except Exception as e:
            print(f"    BSE deliv parse err {dt_str}: {e}")

    # ── Step 3: Join via ISIN bridge → NSE SYMBOL + DELIV_PER ────────────────
    nse_isin_map = globals().get("_NSE_ISIN_MAP", {})

    # Path A: full ISIN bridge (best quality)
    if nse_isin_map and isin_to_scrip and scrip_to_deliv:
        rows = []
        for isin, nse_sym in nse_isin_map.items():
            scrip_cd = isin_to_scrip.get(isin)
            if scrip_cd is None: continue
            dp = scrip_to_deliv.get(int(scrip_cd))
            if dp is None or pd.isna(dp) or not (0 < dp <= 100): continue
            rows.append({"SYMBOL": nse_sym, "CLOSE": np.nan,
                         "DELIV_PER": round(float(dp), 2), "DATE": dt_str})
        if len(rows) > 100:
            return pd.DataFrame(rows)

    # Path B: compute from BSE bhav DELIV_QTY if available
    if bse_bhav_df is not None and nse_isin_map:
        try:
            if "DELIV_QTY" in bse_bhav_df.columns and "VOLUME" in bse_bhav_df.columns:
                bse_bhav_df = bse_bhav_df.copy()
                bse_bhav_df["DELIV_PER"] = (
                    pd.to_numeric(bse_bhav_df["DELIV_QTY"], errors="coerce") /
                    pd.to_numeric(bse_bhav_df["VOLUME"],    errors="coerce") * 100
                ).round(2)
                rows = []
                for _, row in bse_bhav_df.iterrows():
                    isin = str(row.get("ISIN", "")).strip()
                    nse_sym = nse_isin_map.get(isin)
                    if not nse_sym: continue
                    dp = row.get("DELIV_PER")
                    if pd.isna(dp) or not (0 < dp <= 100): continue
                    rows.append({"SYMBOL": nse_sym, "CLOSE": row.get("CLOSE", np.nan),
                                 "DELIV_PER": round(float(dp), 2), "DATE": dt_str})
                if len(rows) > 100:
                    return pd.DataFrame(rows)
        except Exception as e:
            print(f"    BSE path-B err {dt_str}: {e}")
...
def chk_s2(ltp,mcap,rv,l52,h52,d20,d50,nd,vr,tv,s20,s50,s200,s200_rising):
    ...
    if rv   is None or rv < S2_RSI_MIN:                          return False  # RSI > 40 only
    if s20  is None or ltp<=s20:                             return False  # Close > 20 DMA
    if s50  is None or ltp<=s50:                             return False  # Close > 50 DMA
    # SOFT
    if s200 is not None and ltp<=s200:                       return False  # Close > 200 DMA
    if s200_rising is False:                                 return False  # 200 DMA Rising
...
def grade(sc,st,d20,d50,vr,fp,nd,ltp,s20,s50,s200,h60):
    ...
    if st=="s1":
        ...
        above_all = bool(ltp and s20 and s50 and s200
                         and ltp>s20 and ltp>s50 and ltp>s200)
        if dok:
            if sc>=82 and d20>=55 and gap>=8 and vr and vr>=1.5 and above_all:  return "A+"
            if sc>=68 and d20>=48 and gap>=3 and vr and vr>=1.35:               return "A"
        else:
            if sc>=82 and vr and vr>=1.5  and above_all: return "A+"
...
print("="*60)
print("MATRIX 18.2 — INSTITUTIONAL ACCUMULATION SCREENER")
...
print("RSI ceiling removed — penalised in confidence: 70-80=8pts 80-85=3pts >85=0pts")
...
# ── Build ISIN → NSE Symbol map (used by fetch_full for BSE bridge) ───────────
_NSE_ISIN_MAP = {}
for _ds in sorted(bhav_frames.keys(), reverse=True):
    _bf = bhav_frames[_ds]
    if "ISIN" in _bf.columns and "SYMBOL" in _bf.columns:
        _tmp = _bf[["SYMBOL","ISIN"]].dropna()
        _tmp = _tmp[_tmp["ISIN"].astype(str).str.match(r"^IN[A-Z0-9]{10}$")]
        _NSE_ISIN_MAP = dict(zip(
            _tmp["ISIN"].astype(str).str.strip(),
            _tmp["SYMBOL"].astype(str).str.strip().str.upper()
        ))
        if len(_NSE_ISIN_MAP) > 100: break
print(f"  ISIN map: {len(_NSE_ISIN_MAP)} NSE symbols (for BSE delivery bridge)")

# ── Phase 2: BSE Delivery data 50 days ───────────────────────────────────────
full_dates=trdates(DAYS_FULL)
print(f"\nPhase 2: {len(full_dates)} days delivery (BSE — ISIN bridge)...")
full_frames={}
...
print(f"  Loaded: {del_days} days (BSE gross deliverables via ISIN bridge)")
if full_frames:
...
for sym in symbols:
    try:
        ...
        score = sc_s1(d20,d50,vr,rv,fp,tv) if st=="s1" \
                else sc_s2(d20,d50,vr,rv,ltp,s20,s50,s200,tv)
...
print("="*60)
print(f"MATRIX 18.2 DONE — {len(results)} stocks | S1:{s1n} S2:{s2n}")
