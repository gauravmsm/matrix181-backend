"""
MATRIX 18.3 — NSE Institutional Accumulation Screener
======================================================
...
Key change vs 18.2:
  S2 soft filter #9 (52W High proximity 2-45%) REMOVED.
  Replaced with 200DMA Distance Score component in sc_s2() and conf():
    2-10% above 200DMA  → 15/15 pts  (ideal institutional zone)
    10-20% above 200DMA → 12/15 pts  (healthy trend)
    20-35% above 200DMA →  8/15 pts  (extended but valid)
    0-2%  above 200DMA  →  5/15 pts  (just crossing)
    below 200DMA        →  0/15 pts  (downtrend — penalised)
  Score capped at 8/15 if 200DMA slope is falling (sma200_rising=False).
  Rationale: measures where price IS vs trend (200DMA) not where it WAS (52W High).
  Eliminates value traps; rewards early accumulation zone.
"""
import pandas as pd
import numpy as np
...
S2_VOL_RATIO = 1.10
S2_TV_MIN    = 25.0
# 200DMA Distance Score thresholds (replaces 52W High filter from 18.2)
# Score is capped at 8/15 if 200DMA slope is falling
S2_DMA200_IDEAL_LO  =  2.0   # 2% above 200DMA  → start of ideal zone
S2_DMA200_IDEAL_HI  = 10.0   # 10% above 200DMA → end of ideal zone
S2_DMA200_EXT_HI    = 20.0   # 10-20% → healthy; 20-35% → extended
S2_DMA200_MAX       = 35.0   # >35% → still scores 8 (not penalised further)
S2_DEL_MIN   = 40.0
...
def chk_s2(ltp,mcap,rv,l52,h52,d20,d50,nd,vr,tv,s20,s50,s200,s200_rising):
    ...
    if rv   is None or rv < S2_RSI_MIN:                      return False  # RSI > 40
    ...
    # NOTE: 52W High proximity filter REMOVED in 18.3.
    # Replaced by 200DMA Distance Score in sc_s2() and conf().
    # Rationale: 52W High measures distance from past peak (where price WAS),
    # not trend quality (where price IS). Value traps pass it; early
    # accumulation setups score better with 200DMA distance.
    if nd>=10 and d20 is not None:
        if d20<S2_DEL_MIN:                                   return False
    return True

# ── Scoring ───────────────────────────────────────────────────────────────────
def dma200_score(ltp, s200, s200_rising, max_pts=15):
    """
    Score 0-15 based on distance of Close above 200DMA.
    Replaces the binary 52W High soft filter in S2.

    Zones:
      2-10%  above 200DMA → 15  (ideal institutional accumulation zone)
      10-20% above 200DMA → 12  (healthy trend)
      20-35% above 200DMA →  8  (extended but valid)
      0-2%   above 200DMA →  5  (just crossing — early signal)
      below  200DMA       →  0  (downtrend — penalised)

    Cap at 8 if 200DMA slope is falling (protects against dead-cat bounces
    above a declining 200DMA).
    """
    if ltp is None or s200 is None or s200 <= 0:
        return 0
    pct = (ltp - s200) / s200 * 100   # % above 200DMA
    if pct < 0:
        score = 0
    elif pct < 2:
        score = 5
        ...
        score = 8   # >35% — still valid, not penalised further
    # Cap at 8 if 200DMA is falling (s200_rising=False, not None)
    if s200_rising is False:
        score = min(score, 8)
    return score
...
def sc_s2(d20,d50,vr,rv,ltp,s20,s50,s200,tv,s200_rising=None):
    """
    S2 scoring — 18.3 change:
    200DMA Distance Score (0-15) replaces old SMA alignment bonus (3×5=15).
    Rationale: graduated 200DMA distance is more precise than a binary
    above/below check for each MA level.
    """
    s=0.0
    if d20 and d20>=40: s+=min((d20-40)/40*25,25)
    if d20 and d50 and(d20-d50)>2: s+=min((d20-d50-2)/13*20,20)
    if vr  and vr>=1.1: s+=min((vr-1.1)/0.9*20,20)
    if rv:  s+=max(0,20-abs(rv-55)*0.8)
    # 200DMA Distance Score (0-15) — replaces old 3×5 SMA bonus
    s += dma200_score(ltp, s200, s200_rising, max_pts=15)
...
def conf(d20,d50,dlat,vr,rv,fp,sc,gr,dvals,ltp,s20,s30,s50,s150,s200,st,
    ...
    # S2: 200DMA Distance Score — 18.3 change (replaces old 3×5 SMA check)
    rr=0
    if st=="s1" and fp:
        if 10<=fp<=35:  rr=15
        elif 5<=fp<10:  rr=11
        ...
        rr = dma200_score(ltp, s200, s200_rising, max_pts=15)
...
print("="*60)
print("MATRIX 18.3 — INSTITUTIONAL ACCUMULATION SCREENER")
...
print("S2 CHANGE: 52W High filter removed → 200DMA Distance Score (0-15pts)")
...
print("="*60)
print(f"MATRIX 18.3 DONE — {len(results)} stocks | S1:{s1n} S2:{s2n}")
