...
def compute_atr(sdf, n=20):
    ...
    """
    try:
        highs  = sdf["HIGH"].dropna().values  if "HIGH"  in sdf.columns else None
        lows   = sdf["LOW"].dropna().values   if "LOW"   in sdf.columns else None
        cls    = sdf["CLOSE"].dropna().values
        if highs is None or lows is None or len(cls) < n + 1:
            return None
        tr_vals = []
        for i in range(1, min(len(cls), len(highs), len(lows))):
            h, l, pc = float(highs[i]), float(lows[i]), float(cls[i-1])
