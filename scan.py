...
def fetch_full_bhavdata(date):
    ...
    Contains: SYMBOL, SERIES, OPEN, HIGH, LOW, CLOSE, LAST, PREVCLOSE,
              TOTTRDQTY, TOTTRDVAL, TIMESTAMP, TOTALTRADES,
              ISIN, DELIV_QTY, DELIV_PER
    DELIV_PER = delivery percentage — exactly what we need!
    """
    d = date.strftime("%d%m%Y")
    url = f"https://nsearchives.nseindia.com/products/content/sec_bhavdata_full_{d}.csv"
    try:
        r = SESSION.get(url, timeout=20)
        print(f"  sec_bhavdata_full {date.strftime('%d-%m-%Y')} → {r.status_code} len={len(r.content)}")
        ...
        print(f"  sec_bhavdata_full error: {e}")
    return None

def fetch_52w_report(date):
    """
    NSE '52 Week High Low Report'
    URL: https://nsearchives.nseindia.com/content/cm/CM_52_wk_High_low_DDMMYYYY.csv
    Contains exact 52-week high/low for each symbol.
    """
    d = date.strftime("%d%m%Y")
    url = f"https://nsearchives.nseindia.com/content/cm/CM_52_wk_High_low_{d}.csv"
    try:
        r = SESSION.get(url, timeout=20)
        print(f"  52W report {date.strftime('%d-%m-%Y')} → {r.status_code} len={len(r.content)}")
        if r.status_code != 200 or len(r.content) < 1000:
            return {}
        from io import StringIO
        for enc in ["utf-8", "latin-1"]:
            try:
                df = pd.read_csv(StringIO(r.content.decode(enc)))
                df.columns = df.columns.str.strip().str.upper()
                print(f"  52W cols: {list(df.columns)}")
                ...
                if sym_col and high_col and low_col:
                    ...
                    print(f"  52W data: {len(result)} symbols | sample: {list(result.items())[:3]}")
                    return result
            except: continue
    except Exception as e:
        print(f"  52W error: {e}")
...
def normalise_bhav(df):
    ...
    if "VOLUME"  not in df.columns:
        for c in ["TTLTRADGVOL","TOTTRDQTY"]:
            if c in df.columns: rm[c]="VOLUME"; break
    if "VALUE"   not in df.columns:
        for c in ["TTLNBOFTXSEXCTD","TTLBOFTXSEXCTD","TOTTRDVAL"]:
...
def check_s1(ltp, mcap, rsi, high_52w, low_52w, avg_d20, avg_d50,
    ...
    if low_52w and low_52w > 0:
        mult = ltp / low_52w
        if not (S1_FROM_LOW_MIN <= mult <= S1_FROM_LOW_MAX): return False
...
def check_s2(ltp, mcap, rsi, high_52w, low_52w, avg_d20, avg_d50,
    ...
    if high_52w and high_52w > 0 and ltp > high_52w * S2_FROM_HIGH_MAX: return False
...
# ── Fetch latest trading day's full bhavcopy + delivery ───────────────────────
trading_dates = get_trading_dates(10)
latest_date   = None
full_bhav_df  = None
w52_map       = {}

print("\nFetching full bhavdata + 52W report...")
for date in trading_dates:
    df = fetch_full_bhavdata(date)
    if df is not None and len(df) > 100:
        full_bhav_df = df
        latest_date  = date
        # Also fetch 52W report for same date
        w52_map = fetch_52w_report(date)
        print(f"  Using date: {date.strftime('%Y-%m-%d')}")
        break

# ── Extract delivery from full bhavdata ───────────────────────────────────────
delivery_today = {}
latest_close_full = {}

if full_bhav_df is not None:
    print(f"\nFull bhavdata cols: {list(full_bhav_df.columns)}")
    print(f"Sample row: {dict(list(full_bhav_df.iloc[0].items())[:10])}")

    eq_mask = full_bhav_df.get("SERIES", pd.Series(["EQ"]*len(full_bhav_df))) == "EQ"
    fd_eq   = full_bhav_df[eq_mask].copy() if eq_mask.sum() > 0 else full_bhav_df.copy()

    sym_col = next((c for c in fd_eq.columns if c=="SYMBOL"), None)

    # Delivery percent column
    del_col = next((c for c in fd_eq.columns if "DELIV" in c and "PER" in c), None)
    if del_col is None:
        del_col = next((c for c in fd_eq.columns if "DELIV" in c), None)

    print(f"  Symbol col: {sym_col} | Delivery col: {del_col}")

    if sym_col and del_col:
        dpct = pd.to_numeric(fd_eq[del_col], errors="coerce")
        syms = fd_eq[sym_col].astype(str).str.strip().str.upper()
        for idx in fd_eq.index:
            sym = syms[idx]
            p   = dpct[idx]
            if pd.notna(p) and 0 < p <= 100:
                delivery_today[sym] = round(float(p), 2)
        print(f"  Delivery today: {len(delivery_today)} symbols ✓")
        if delivery_today:
            print(f"  Sample: {list(delivery_today.items())[:3]}")

    # Close prices
    cl_col = next((c for c in fd_eq.columns if c=="CLOSE"), None)
    if sym_col and cl_col:
        cl = pd.to_numeric(fd_eq[cl_col], errors="coerce")
        for idx in fd_eq.index:
            sym = syms[idx]
            c   = cl[idx]
            if pd.notna(c) and c > 0:
                latest_close_full[sym] = float(c)

# ── Phase 1: 60 days bhavcopy for OHLCV + SMAs ────────────────────────────────
detail_dates = get_trading_dates(DAYS_DETAIL)
print(f"\nPhase 1: {len(detail_dates)} days...")
...
if delivery_today and latest_date:
    delivery_map[latest_date.strftime("%Y-%m-%d")] = delivery_today
    print(f"  Added today's delivery: {len(delivery_today)} symbols")

for i, date in enumerate(detail_dates):
    try:
        # Try full bhavdata first (has delivery built in)
        if date == latest_date and full_bhav_df is not None:
            bhav = full_bhav_df.copy()
            if "SYMBOL" not in bhav.columns:
                bhav = normalise_bhav(bhav)
        else:
        ...
        # Extract delivery from full bhav if available for this date
        if ds not in delivery_map:
            for del_col in ["DELIV_PER","DELIV_PCT","DELIVERY_PER","DELIVERY_PCT"]:
                if del_col in bhav_eq.columns and "SYMBOL" in bhav_eq.columns:
                    dpct = pd.to_numeric(bhav_eq[del_col], errors="coerce")
                    syms = bhav_eq["SYMBOL"].astype(str).str.strip().str.upper()
                    mask = (dpct > 0) & (dpct <= 100)
                    if mask.sum() > 50:
                        dd = {syms[idx]: round(float(dpct[idx]),2) for idx in bhav_eq[mask].index}
                        if dd:
                            delivery_map[ds] = dd
                            print(f"  Delivery {ds}: {len(dd)} ✓")
                    break

        # Value
        if "VALUE" in bhav_eq.columns and "SYMBOL" in bhav_eq.columns:
            v_num = pd.to_numeric(bhav_eq["VALUE"], errors="coerce")
            if "TOTTRDVAL" in bhav_eq.columns:
                v_num = pd.to_numeric(bhav_eq["TOTTRDVAL"], errors="coerce")
            syms  = bhav_eq["SYMBOL"].astype(str).str.strip().str.upper()
            vd = {}
            for idx in bhav_eq.index:
                v = v_num[idx] if idx in v_num.index else np.nan
                if pd.notna(v) and v > 0:
                    # full bhav value is in crores already; bhavcopy is in lakhs
                    vd[syms[idx]] = float(v)/100 if v > 1000 else float(v)
        ...
        if i == 0 and "CLOSE" in bhav_eq.columns and "SYMBOL" in bhav_eq.columns:
            ...
            for idx in bhav_eq.index:
                ...
                if pd.notna(c) and c > 0: latest_close[syms[idx]] = float(c)
...
# MCap
close_map_for_mcap = latest_close_full if latest_close_full else latest_close
print(f"\nLoading MCap ({len(close_map_for_mcap)} closes)...")
mcap_map   = fetch_mcap_from_equity_list(close_map_for_mcap)
mcap_avail = len(mcap_map) > 0

# Phase 2 — extra closes for SMA150/200
...
w52_avail   = len(w52_map) > 0

print(f"\n{'='*40}")
print(f"Bhavcopy : {actual_days}")
print(f"Delivery : {len(delivery_map)} days, {len(delivery_today)} symbols today {'✓' if del_avail else '✗'}")
print(f"Value    : {len(value_map)} {'✓' if val_avail else '✗'}")
print(f"MCap     : {len(mcap_map)} {'✓' if mcap_avail else '✗'}")
print(f"52W      : {len(w52_map)} {'✓' if w52_avail else '✗'}")
...
for symbol in symbols:
    try:
        ...
        # 52W from dedicated report (most accurate) or fallback to bhavcopy HIGH/LOW
        if w52_avail and sym_u in w52_map:
            high_52w = w52_map[sym_u]["high"]
            low_52w  = w52_map[sym_u]["low"]
        elif "HIGH" in sdf.columns and "LOW" in sdf.columns:
            high_52w = round(float(sdf["HIGH"].max()),2)
            low_52w  = round(float(sdf["LOW"].min()),2)
        else:
            high_52w = round(float(all_c.max()),2) if len(all_c)>0 else ltp
            low_52w  = round(float(all_c.min()),2) if len(all_c)>0 else ltp

        from_low_pct  = round((ltp-low_52w)/low_52w*100,2)   if low_52w>0 else None
        from_high_pct = round((high_52w-ltp)/high_52w*100,2) if high_52w>0 else None
        ...
        strategy = None
        if check_s1(ltp,mcap,rsi,high_52w,low_52w,avg_d20,avg_d50,
                    vol_ratio,avg_tv,sma45,sma150,del_avail,val_avail):
            strategy = "s1"
        elif check_s2(ltp,mcap,rsi,high_52w,low_52w,avg_d20,avg_d50,
                      vol_ratio,avg_tv,sma20,sma50,sma200,del_avail,val_avail):
            strategy = "s2"
        ...
        results.append({
            "symbol":str(symbol), "strategy":strategy,
            "strategyLabel":"Mid/Small Cap" if strategy=="s1" else "Large/Mega Cap",
            "ltp":ltp, "change":change, "high52w":high_52w, "low52w":low_52w,
            "fromLow":from_low_pct, "fromHigh":from_high_pct, "volume":vol_today,
            "mcap":round(mcap,0) if mcap else None,
            "tradedValueCr":round(avg_tv,2) if avg_tv else None,
            "rsi":rsi, "sma9":sma9, "sma20":sma20, "sma45":sma45, "sma50":sma50,
            "sma150":sma150, "sma200":sma200, "volRatio":vol_ratio,
            "avgDelivery":avg_d20, "avgDelivery50":avg_d50,
            "deliveryTrend":round(avg_d20-avg_d50,2) if avg_d20 and avg_d50 else None,
            "deliveryToday":lat_del, "score":score, "grade":grade,
            "daysOfData":len(sdf), "confidence":conf,
            "confidenceLabel":clabel, "confidenceBreakdown":sigs,
        })
    except Exception as e:
...
output = {
    ...
    "dataAvailability":{"mcap":mcap_avail,"delivery":del_avail,"value":val_avail,"sma200":len(extra_list)>0},
}
