...
print("\nPhase 0: Fetching global market data...")
...
print(f"  VIX: {vix_val} | Regime: {regime}")

# Nifty 50 closes for RS computation
nifty_closes = fetch_nifty_closes(260)
print(f"  Nifty: {'OK' if nifty_closes else 'unavailable'} ({len(nifty_closes) if nifty_closes else 0} days)")
...
for sym in symbols:
    try:
        ...
        # ── CPM ───────────────────────────────────────────────────────────────
        deploy_pct, cpm_label = compute_cpm(regime, struct_score, gr)
...
print("="*60)
print(f"MATRIX 18.5 DONE — {len(results)} stocks | S1:{s1n} S2:{s2n} | VIX:{vix_val} | Regime:{regime}")
